...
public class PanelUpdM extends JPanel{
    ...
    /**
     * Pannello per la modifica di un esame composto.
     * @author Simone Bonini
     */
    
	public PanelUpdM(int numEs, String nome, String corso, int[] voti, double[] pesi, int cfu) {
        ...
        spinnerVoti = new JSpinner[numEs];
        spinnerPesi = new JSpinner[numEs];
        
        for (int i = 0; i < numEs; i++) {
        	spinnerVoti[i] = new JSpinner(new SpinnerNumberModel(voti[i], 18, 30, 1)); 
        	spinnerPesi[i] = new JSpinner(new SpinnerNumberModel(pesi[i], 0.01, 0.99, 0.05)); 
